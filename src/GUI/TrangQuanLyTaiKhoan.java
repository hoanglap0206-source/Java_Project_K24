...
public class TrangQuanLyTaiKhoan extends JPanel {
    ...
    private void loadDataFromBUS() {
        ...
        // Sắp xếp theo số trong mã NV (NV01, NV02, ... NV88, NV99, NV1000000)
        ArrayList<NhanVien> list = nvBUS.getAll();
        list.sort((a, b) -> {
            int numA = tachSoTuMa(a.getMaNV());
            int numB = tachSoTuMa(b.getMaNV());
            if (numA != numB) return Integer.compare(numA, numB);
            return a.getMaNV().compareToIgnoreCase(b.getMaNV());
        });
    ...
    /** Tách phần số ở cuối mã, ví dụ "NV1000000" → 1000000, "NV01" → 1 */
    private int tachSoTuMa(String ma) {
        try {
